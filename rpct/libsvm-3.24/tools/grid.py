...
def find_parameters(dataset_pathname, options=''):
	...
	def update_param(c,g,rate,best_c,best_g,best_rate,worker,resumed):
		...
		#print('\r'+ stdout_str,end='',flush=True)#这里被我注释掉了，去掉注释并去掉'\r'会在窗口显示超参数寻优详细信息
		if options.out_pathname and not resumed:
			output_str += 'rate={0}\n'.format(rate)
			#result_file.write(output_str)#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
			#result_file.flush()#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
	...
	#if options.out_pathname:#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
		#if options.resume_pathname:#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
			#result_file = open(options.out_pathname, 'a')#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
		#else:#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
			#result_file = open(options.out_pathname, 'w')#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
	...
	for line in jobs:
		...
		#if gnuplot and options.grid_with_c and options.grid_with_g:#这里被我注释掉了，去掉注释会打开gnuplot窗口，显示超参数寻优具体进程，并且输出超参数寻优结果图片
			#redraw(db,[best_c, best_g, best_rate],gnuplot,options)
			#redraw(db,[best_c, best_g, best_rate],gnuplot,options,True)


	#if options.out_pathname:#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
		#result_file.close()#这里被我注释掉了，去掉注释会生成超参数寻优结果文件
	...
	if best_g != None:
		...
		best_cg += [2.0**best_g]
	print('\n>>>c = '+ str(best_cg[0]) + '  gamma = '+ str(best_cg[1]) + '  ACC = {1}%\n'.format(' '.join(map(str,best_cg)), best_rate))
