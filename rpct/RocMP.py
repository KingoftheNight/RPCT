...
def PCA_change(result_matrix):
    mid_array = list(result_matrix)
    mid_list = []
    for line in mid_array:
        mid_box = []
        for j in line:
            mid_box.append(float(j))
...
def PCA_main(in_path, out_path, c_number, gamma, crossV, now_path):
    ...
    features_index, features_data = PCA_read(in_path)
    # PCA
    features_array, fs_matrix, E1 = PCA_train(features_data)
    len_fs = len(features_data[0])
    # 降维
    all_fs_acc = []
    for i in range(len_fs - 1):
        PCA_time(i, len_fs - 1)
        select_matrix = fs_matrix[:, E1[0:i]]
        result_matrix = np.dot(features_array, select_matrix)
        result_list = PCA_change(result_matrix)
        # 输出特征文件
        mid_ifs = PCA_file(result_list, features_index, now_path)
        # 交叉验证
...
# 读取文件
def roc_filter_read(roc_filename, num):
    with open(roc_filename, 'r') as f1:
        file = f1.readlines()
        f1.close()
    # 提取特征list
    ...
    for i in file:
        line = i.strip('\n').split(' ')
        fs_box = roc_filter_check(line[1:], num)
        # fs_box = line[1:]
        mid_box = []
        for j in fs_box:
            mid_box.append(float(j.split(':')[-1]))
...
# ROC CURVE COMMAND
def roc_graph(file, out, num, c_number, ga, now_path):
    if num == '400':
        roc_features = roc_read(os.path.join(now_path, file))
    else:
        roc_features = roc_filter_read(os.path.join(now_path, file), num)
