...
def messagebox_help_precaution():
    ...
    help_precaution_mg.geometry('640x300')
...
def gui_extract():
    ...
    ex_o = e_ex_o.get()
    ex_l = e_ex_l.get()
    if len(ex_f1) != 0 and len(ex_f2) != 0 and len(ex_r) != 0 and len(ex_o) != 0 and len(ex_l) != 0:
...
def gui_filter():
    ...
    fi_o = e_fi_o.get()
    fi_r = e_fi_r.get()
    if len(fi_f) != 0 and len(fi_c) != 0 and len(fi_g) != 0 and len(fi_cv) != 0 and len(fi_n) != 0 and len(fi_o) != 0 and len(fi_r) != 0:
        print('\n>>>Filter Features...\n')
        TrainFP.filter_pro(fi_f, fi_o, fi_c, fi_g, fi_cv, fi_n, fi_r, now_path)
        v_command = 'filter ' + fi_f + ' -c ' + fi_c + ' -g ' + fi_g + ' -cv ' + fi_cv + ' -n ' + fi_n + ' -o ' + fi_o + ' -r ' + fi_r
        var.set(v_command)
        o_m = n_var.get()
        n_var.set(o_m + '\n' + v_command)
...
def gui_fffs():
    ...
    fs_o = e_fs_o.get()
    fs_l = e_fs_l.get()
    if len(fs_f) != 0 and len(fs_i) != 0 and len(fs_n) != 0 and len(fs_o) != 0 and len(fs_l) != 0:
        print('\n>>>Filter Features File Setting...\n')
        ReadBE.read_filter(fs_f, fs_i, fs_o, fs_n, fs_l, now_path)
        v_command = 'fffs ' + fs_f + ' -f ' + fs_i + ' -n ' + fs_n + ' -o ' + fs_o + ' -l ' + fs_l
        var.set(v_command)
        o_m = n_var.get()
        n_var.set(o_m + '\n' + v_command)
...
def gui_roc():
    ...
    roc_c = e_roc_c.get()
    roc_g = e_roc_g.get()
    if len(roc_f) != 0 and len(roc_o) != 0 and len(roc_n) != 0 and len(roc_c) != 0 and len(roc_g) != 0:
        print('\n>>>Drawing ROC curve...\n')
        RocMP.roc_graph(roc_f, roc_o, roc_n, roc_c, roc_g, now_path)
        v_command = 'roc ' + roc_f + ' -o ' + roc_o + ' -n ' + roc_n + ' -c ' + roc_c + ' -g ' + roc_g
        var.set(v_command)
        o_m = n_var.get()
        n_var.set(o_m + '\n' + v_command)
...
e_ex_r = tk.Entry(cef_2,show=None,width=12,font=('SimHei', 11))
...
e_ex_o = tk.Entry(cef_2,show=None,width=19,font=('SimHei', 11))
e_ex_o.pack(side='left')
tk.Label(cef_2,text='',width=2,anchor='w').pack(side='left')
######lmda
tk.Label(cef_2,text='lmda',width=5,anchor='w').pack(side='left')
e_ex_l = tk.Entry(cef_2,show=None,width=5,font=('SimHei', 11))
...
tk.Label(cff_1,text='',width=2,anchor='w').pack(side='left')
######features number
tk.Label(cff_2,text='features number',width=14,anchor='w').pack(side='left')
e_fi_n = tk.Entry(cff_2,show=None,width=4,font=('SimHei', 11))
e_fi_n.pack(side='left')
...
######end feature
tk.Label(cfsf_2,text='features number',width=16,anchor='w').pack(side='left')
e_fs_l = tk.Entry(cfsf_2,show=None,width=4,font=('SimHei', 11))
e_fs_l.pack(side='left')
tk.Label(cfsf_2,text='',width=2,anchor='w').pack(side='left')
######button
